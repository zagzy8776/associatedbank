        ...
        <div className="max-w-6xl mx-auto px-6 h-9 flex items-center justify-between text-[11px] tracking-wide text-slate-500">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-1.5">
              <Landmark className="w-3.5 h-3.5 text-amber-500/80" /> SWIFT: RBNKGB2L
            </span>
            <span className="flex items-center gap-1.5">
              <Headphones className="w-3.5 h-3.5 text-amber-500/80" /> Client services: +44 (0) 20 7946 0958
            </span>
          </div>
      ...
      <section id="about" className="scroll-mt-24 py-16 border-t border-white/5">
        <div className="max-w-6xl mx-auto px-5 sm:px-6 grid md:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <p className="mt-4 text-slate-400 leading-relaxed">
              Rubicon Capital is built for clients who need sterling, dollars, and euros without juggling three unrelated apps.
              We keep the interface quiet, the numbers honest, and the controls in your hands.
            </p>
          </div>
          <div id="learn" className="scroll-mt-24 space-y-4">
            {[
              'How multi-currency accounts work',
              'Opening an account and getting your number',
              'Transfers, deposits, and statements',
            ].map((title) => (
              <div
                key={title}
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-6 py-10 flex flex-col md:flex-row md:items-start justify-between gap-8">
          <div>
            ...
            <p className="mt-3 text-xs text-slate-500 max-w-xs leading-relaxed">
              Multi-currency banking for private and business clients. SWIFT RBNKGB2L.
            </p>
          </div>
          <div className="flex flex-wrap gap-x-8 gap-y-2 text-xs text-slate-500">
            {['Member Services', 'About', 'Terms', 'Privacy', 'Contact'].map((l) => (
              <a key={l} href="#about" onClick={(e) => goTo(e, '#about')} className="hover:text-amber-400 transition">
                {l}
              </a>
            ))}
          </div>
