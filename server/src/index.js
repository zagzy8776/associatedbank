...
app.post('/api/transfer', authMiddleware, async (req, res) => {
  try {
    ...
    if (!from_account_id || !to_account_id || !amt || amt <= 0) return res.status(400).json({ error: 'Invalid transfer data' });
    const result = await withTransaction(async (client) => {
      const fromRes = await client.query(`SELECT * FROM accounts WHERE id = $1 AND user_id = $2 FOR UPDATE`, [from_account_id, req.user.id]);
      if (!fromRes.rows.length) throw new Error('Source account not found');
      const from = fromRes.rows[0];
      if (from.is_locked || from.status !== 'active') throw new Error('Source account is locked or inactive');
      ...
      if (to.is_locked || to.status !== 'active') throw new Error('Destination account is locked or inactive');
      if (from.currency !== to.currency) throw new Error('Cross-currency transfers not supported');
      ...
      try {
        await client.query(
          `INSERT INTO transactions (account_id, user_id, type, amount, currency, status, description, reference) VALUES ($1,$2,'transfer',$3,$4,'completed',$5,$6)`,
          [from_account_id, req.user.id, amt, from.currency, description || `Transfer to ${to.account_number}`, ref]
        );
      } catch (_) {
        await client.query(
          `INSERT INTO transactions (account_id, type, amount, currency, status, description, reference) VALUES ($1,'transfer',$2,$3,'completed',$4,$5)`,
          [from_account_id, amt, from.currency, description || `Transfer to ${to.account_number}`, ref]
        );
      }
